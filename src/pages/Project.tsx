...
class Project extends Component<IProjectProps, IProjectState> {
    ...
    generateMetaTags = (project: any) => {
        ...
        // Format team members
        const teamMembers = project.members.map((m: any) => m.member.name).join(', ');
        
        // Status text
        const status = project.endDate ? 'Completed' : 'In Progress';
        
        // Enhanced description with more details
        const enhancedDescription = `${description} | Built with ${project.tags.slice(0, 3).join(', ')} | Team: ${teamMembers} | Status: ${status}`;

        return {
            title: `${project.name} - Project Showcase`,
            description: enhancedDescription,
            keywords: `${keywords}, project, portfolio, development, ${project.category?.name || 'software'}`,
                ...
                <Helmet>
                    ...
                    <script type="application/ld+json">
                        {JSON.stringify({
                            ...
                            "image": project.projectImage,
                            "url": metaTags.url,
                            "author": project.members.map((m: any) => ({
                                "@type": "Person",
                                "name": m.member.name,
                                "jobTitle": m.role
                            })),
